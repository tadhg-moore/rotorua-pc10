...
# Draft Data Request List — Catchment Modelling Project
...
| Dataset | Description / Variables | Purpose in Modelling | Source |
|----|----|----|----|
| **Inflow volumes** | Daily/hourly flow records for all tributaries and stormwater inputs | Define hydrological boundary conditions; drive water balance | Same as above |
| **Inflow chemistry** | Nutrient concentrations (TN, TP, NO3, NH4, DRP), organic fractions, conductivity, temperature | Set up inflow nutrient loads and biogeochemical forcing | Same as above |
| **Grab samples & rating curve data** | Flow–concentration relationships, historical spot samples | Improve accuracy of nutrient load estimates | Same as above |
| **Meteorological data (buoy)** | Wind speed/direction, air temp, humidity, shortwave/longwave radiation, rainfall | Primary atmospheric forcing for lake model | Same as above |
| **Meteorological data (airport)** | NIWA climate station data (same variables as above) | Gap filling and long-term climate records | Same as above |
| **CMIP6 climate data** | Bias-corrected scenario climate inputs | Future scenario modelling | Same as above |
| **Lake level / stage** | Time series of lake height | Convert model depths to dynamic hypsography; align inflow/outflow | Same as above |
| **Catchment shapefile** | Full catchment boundary polygon | Define drainage area for inflow and land-use analysis | Same as above |
| **Sub-catchment delineations** | Individual subcatchment polygons | Attribute inflows to specific contributing areas | Same as above |
| **Land-use data (current)** | Latest regional council land-use dataset | Estimate nutrient export, identify land-use pressures | Same as above |
| **Historical land-use (LCDB)** | LCDB time-series land-cover data | Identify changes over time influencing nutrient loading | Same as above |
| **ROTAN model outputs** | Rotorua/Taupō nitrate model datasets (flows, loads) | Cross-check nutrient source estimates; historic nitrate loading | Same as above |
| **Point-source inputs** | Any wastewater/stormwater discharge records | Ensure all external loads are captured | Same as above |
